...
const IcebergOrder = ({ orderType, stock }) => {
  if (!stock) return null;

  const [qty, setQty] = useState(1);
  const [price, setPrice] = useState(stock?.price || 22.94);
  const [iceberg, setIceberg] = useState(1);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center">
        <div className="w-20 text-sm font-medium text-gray-600">Iceberg</div>
        <input
          type="number"
          value={iceberg}
          onChange={(e) => setIceberg(Math.max(1, parseInt(e.target.value) || 1))}
          min="1"
          className="flex-1 px-3 py-1.5 text-right border border-gray-300 rounded text-sm"
        />
      </div>
